...
export default function WorkflowHealth({ repoName, token, julesApiKey }: WorkflowHealthProps) {
  ...
  useEffect(() => {
    if (repo) {
      const cached = storage.getWorkflowAudit(repo);
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      setAnalysis(result);
      storage.saveWorkflowAudit(repo, result);
    } catch (err: any) {
  ...
  const dispatchFullWorkflowAudit = async () => {
    if (!analysis) return;
    
    const body = `## 📊 Workflow Health Audit Report
...
### 🔍 Technical Findings
${analysis.technicalFindings.map(f => `#### ${f.title} (${f.type.toUpperCase()})
- **Description:** ${f.description}
- **Location:** \`${f.location || 'N/A'}\`
- **Remediation:** ${f.remediation}`).join('\n\n')}

### 💡 Qualitative Analysis
- **Efficacy:** ${analysis.qualitativeAnalysis.efficacy}
- **Coverage:** ${analysis.qualitativeAnalysis.coverage}
- **Efficiency:** ${analysis.qualitativeAnalysis.efficiency}

### 🚀 Strategic Recommendations
${analysis.qualitativeAnalysis.recommendations.map((r, i) => `${i + 1}. ${r}`).join('\n')}
                  ...
                  <div className="flex-1 space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <Button
                        size="sm"
                        variant="secondary"
                        onClick={dispatchFullWorkflowAudit}
                        disabled={isDispatching('full-workflow-audit')}
                        isLoading={isDispatching('full-workflow-audit')}
                        icon={Github}
                      >
