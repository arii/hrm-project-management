...
export default function GeminiStatus() {
  ...
  const isRecommended = (model: GeminiModelInfo) => {
    // Explicitly exclude underpowered models from recommendations
    if (model.name.toLowerCase().includes('nano')) return false;
    
    // Exact tier matching
    if (currentTier === ModelTier.PRO) return model.name.toLowerCase().includes('pro');
    if (currentTier === ModelTier.LITE) return model.name.toLowerCase().includes('lite');
    
    // Default to Flash (but not Lite/Pro) for the FLASH tier
    if (currentTier === ModelTier.FLASH) {
      return model.name.toLowerCase().includes('flash') && !model.name.toLowerCase().includes('lite');
    }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500">
      ...
      {testing && (
        <div className="bg-slate-900 border border-indigo-500/30 rounded-2xl overflow-hidden shadow-lg animate-in slide-in-from-top-4 duration-300">
          <div className="p-4 space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Loader2 className="w-4 h-4 text-indigo-400 animate-spin" />
                <span className="text-xs font-bold text-white uppercase tracking-wider">Diagnostic in Progress...</span>
              </div>
              <span className="text-xs font-mono text-indigo-400">{diagnosticProgress}%</span>
            </div>
